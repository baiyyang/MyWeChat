...
# 绘制柱状图
def draw(bins, label, xlabel=None, ylabel=None, title=None, type=1):
    plt.bar(left=range(len(bins)), height=bins, color='rgb', tick_label=label)
    for a, b in zip(range(len(bins)), bins):
        if type:
            plt.text(a, b+0.05, '%d' % b, ha='center', va='bottom', fontsize=10)
        else:
            plt.text(a, b+0.05, '%.2f%%' % b, ha='center', va='bottom', fontsize=10)
    plt.xlabel(xlabel, size='medium')
    plt.ylabel(ylabel, size='medium')
...
wordcloud = WordCloud(mask=bg_pic, font_path='/Library/Fonts/Songti.ttc', background_color='white',
                      max_words=2000, max_font_size=100, min_font_size=10, scale=1.5).\
